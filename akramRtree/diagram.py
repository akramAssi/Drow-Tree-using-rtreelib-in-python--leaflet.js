...
def create_rtree_diagram0(tree: RTreeBase, label=None, fmt="png", filename=None, filename_dot=None,
...
#    graph.set_node_defaults(shape='plaintext')
    graph.set_node_defaults(color='lightgray',
                            style='filled',
                            shape='plaintext',
                            fontname='Courier',
                            fontsize='15')
    ...
    tit = datetime.datetime.now().strftime("%H:%M:%S")
    filename = filename +"' "+ str(tit)+" '.png" or  tempfile.mkstemp('.' + fmt)[1]
...
def _rtree_node_to_pydot(node: RTreeNode, img=None):
    ...
    children_cells = ''.join([f'<td port="{id(entry)}"><font point-size="15">{entry}</font></td>'
    ...
    return pydot.Node(
        ...
        label=f'''<<table border="1" cellborder="1" cellspacing="2">
                <tr><td border="0" colspan="{num_children}"><font point-size="15"><b>{node}</b></font></td></tr>
                <tr><td border="0" colspan="{num_children}"><font point-size="15">rect={rect_str}</font></td></tr>
                <tr><td border="0" colspan="{num_children}"><font point-size="15">area={rect.area()}</font></td></tr>
                {img_row}
...
def _rtree_leaf_entry_to_pydot(entry: RTreeEntry, img=None):
    ...
    data_str = f'<tr><td><font point-size="15">data={entry.data}</font></td></tr>' if entry.is_leaf else None
    return pydot.Node(
        id(entry),
        label=f'''<<table border="1" cellborder="0" cellspacing="0">
                      <tr><td><font point-size="15"><b>{entry}</b></font></td></tr>
                      <tr><td><font point-size="15">rect={rect_str}</font></td></tr>
                      {data_str}
                      {img_row}
                  </table>>'''
    )
